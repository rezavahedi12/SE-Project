...
namespace SemnanFlix.Web.Config
{
    public class Startup
    {
        ...
        public virtual void ConfigureServices(IServiceCollection services)
        {
            ...
            //Note: SemnanFlix uses a local SMTP server, but in production, you'd want to use something like SendGrid, which offers 100 free emails a day: https://sendgrid.com/free/
            services.AddOptions();
            services.Configure<EmailConfiguration>(Configuration.GetSection("Email"));

            //Set up database
            services.AddDbContext<SemnanFlixContext>(options =>
                options.UseSqlServer(Configuration.GetConnectionString("SemnanFlixConnection")));

            //Set up Identity
            services.AddIdentity<ApplicationUser, IdentityRole>()
                .AddEntityFrameworkStores<SemnanFlixContext>()
