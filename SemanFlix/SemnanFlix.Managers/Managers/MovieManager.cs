...
namespace SemnanFlix.Managers.Managers
{
    public class MovieManager : IMovieManager
    {
        ...
        public MoviesViewModel Search(string name)
        {
            ...
            var vms = Mapper.Map<List<MovieViewModel>>(movieDTOs);

            return new MoviesViewModel { Movies = vms.OrderByDescending(x => x.ReleaseDate).ToList() };
        }

        public void RatingAndReview(RatingAndReviewViewModel model)
        {
            //mapper
            var dto = Mapper.Map<RatingAndReviewDTO>(model);

            _movieAccessor.SaveRatingAndReview(dto);
        }

        public List<RatingAndReviewViewModel> getReview(int id)
        {
            var result = _movieAccessor.GetReview(id);
            var vms = Mapper.Map<List<RatingAndReviewViewModel>>(result);
            return vms;
        }
