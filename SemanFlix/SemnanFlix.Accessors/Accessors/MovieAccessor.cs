...
namespace SemnanFlix.Accessors.Accessors
{
    ...
    {
        public MovieAccessor(SemnanFlixContext db) : base(db)
        {
        }
        ...
        public List<RatingAndReviewDTO> GetReview(int id)
        {
            var entities = _db.RatingAndReview.Where(x => x.MovieId == id).ToList();
            var dtos = Mapper.Map<List<RatingAndReviewDTO>>(entities);
            return dtos;
        }
        ...
        public void SaveRatingAndReview(RatingAndReviewDTO model)
        {
            //mapper 
            var entity = Mapper.Map<RatingAndReview>(model);

            _db.RatingAndReview.Add(entity);
            _db.SaveChanges();
